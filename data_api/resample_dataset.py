...
class Data_api_rec(Dataset):
    def __init__(self, root, gen_label=True, updata_txt=True, train_test_ratio=0.8, phase="test",
        ...
        if gen_label:
            ...
            # 载入需要重复采样的文件夹
            self.images_path += glob(os.path.join(self.root, "low_image", '*.bmp'))

            # image会被替换为label，所以其他文件夹名字不要出现image，否则会出现错误路径
            self.images_xml = [image_path.replace(".bmp", ".xml").replace("low_image", "label").replace("image", "label")
    ...
    def make_txt(self, train_test_ratio=0.5):
        ...
        # 对应在数据集文件夹中创建low_label文件夹存放xml标签
        images_path += glob(os.path.join(self.root, "low_image", '*.bmp'))
...
def sampler_c(dataset):
    ...
    sampler1 = sampler.WeightedRandomSampler(weights=samples_weights1, num_samples=3*len(dataset), replacement=True)
    sampler2 = sampler.WeightedRandomSampler(weights=samples_weights2, num_samples=len(dataset), replacement=False)
    return sampler1, sampler2


def main():
    root = "/media/root/软件/wqr/data/sampler"
    dataset = Data_api_rec(root=root, gen_label=False, updata_txt=False,
                           train_test_ratio=0.8, phase="train", use_relative_path=False)
    # 实现同一个batch中从低对比度文件夹中采取n张 正常图片文件夹中采取m张进行模型的训练，一个epoch的大小为不放回采样的batch大小
    sampler1, sampler2 = sampler_c(dataset)
    train_loader1 = torch.utils.data.DataLoader(dataset=dataset, batch_size=3, shuffle=False, sampler=sampler1)
    train_loader2 = torch.utils.data.DataLoader(dataset=dataset, batch_size=1, shuffle=False, sampler=sampler2)
    for data in zip(train_loader1, train_loader2):
        image_tensor = torch.cat([data[0][0], data[1][0]], dim=0)
        label_tensor = torch.cat([data[0][1], data[1][1]], dim=0)
        image_path = data[0][2]+data[1][2]
        image_label = torch.cat([data[0][3], data[1][3]], dim=0)
        print(image_path)

    # label_tensor, image_path, image_label
