...
test_parser.add_argument("--state_path", "-sp", default="checkpoints/network_state/epoch46_acc94.74_model.pth",
...
test_data = Data_api_rec(root=opt.data_root, gen_label=False, updata_txt=False,
...
for i, (x, y, image_path, image_label) in enumerate(tqdm(test_loader)):
    ...
    y = np.uint8(y*255)
    # 可视化图像
    out_cv1 = output.detach().cpu()
    out_cv1 = np.uint8(out_cv1 * 255)
    _, out_cv = cv2.threshold(out_cv1, 128, 255, cv2.THRESH_BINARY)
    concat_image = Concat3CImage([np.uint8(x * 255), y, out_cv], mode='Col', offset=1)
